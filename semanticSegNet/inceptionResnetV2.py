...
def InceptionResNetV2Dec(input):
    x = Conv2DTranspose(1526, (3,3), strides=(2,2), name = 'conv_up_1')(input)
    x = Conv2DTranspose(768, (3,3), strides=(2,2), name = 'conv_up_2')(x)
    x = Conv2DTranspose(384, (3,3), strides=(2,2), name = 'conv_up_3')(x)
    x = Conv2DTranspose(192, (3,3), strides=(2,2), name = 'conv_up_4')(x)
    x = Conv2DTranspose(35, (3,3), strides=(2,2), name = 'conv_up_5')(x)
    x = tf.nn.softmax(x)
    return x

def InceptionResNetV2Model():
    base_model = InceptionResNetV2(include_top=True, weights='imagenet')
    # base_model.summary()
    model = Model(base_model.input, base_model.get_layer('conv_7b_ac').output, name = 'inception_resnet_v2_variation')
    output = InceptionResNetV2Dec(model.output)
    model = Model(model.input, output, name = 'inception_resnet_v2_variation_dec')
