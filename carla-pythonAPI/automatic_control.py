...
# ==============================================================================
	...
	def setup_depth_camera(self):
		...
		self.depth_camera = self.world.spawn_actor(self.camera_blueprint('depth'), depth_camera_transform, attach_to=self.player)
	...
	def setup_semantic_camera(self):
		...
		self.semantic_camera = self.world.spawn_actor(self.camera_blueprint('semantic_segmentation'), semantic_camera_transform, attach_to=self.player)
	...
	def img_render(self, display, count):
		"""
	...
	def depth_render(self, count):	
		if self.depth_image is not None:
			...
			i3 = i2[:, :, :3]
			# self.depth = i3
			cv2.imshow("depth_image", i3)
			cv2.waitKey(1)
	
	def semantic_render(self, count):
	...
	def semantic_save(self, count):
		if self.semantic_image is not None:
			...
			# sem_img = copy.copy(self.semantic_image)
			# sem_img.convert(carla.ColorConverter.CityScapesPalette)
			self.semantic_image.save_to_disk("../carlaData/semantic/idcolor%05d.png" % count, carla.ColorConverter.CityScapesPalette)
...
def game_loop(args):
	...
	try:
		...
		# world.display = pygame.display.set_mode((VIEW_WIDTH, VIEW_HEIGHT), pygame.HWSURFACE | pygame.DOUBLEBUF)

		world.setup_camera()
		world.setup_depth_camera()
		world.setup_semantic_camera()
		...
		world.display = cv2.namedWindow('image')
		world.depth_display = cv2.namedWindow('depth_image')
		world.semantic_display = cv2.namedWindow('semantic_image')
		count = 0
		world.set_synchronous_mode(True)
		while True:
			cv2.waitKey(1)
			world.capture = True
			world.depth_capture = True
			world.semantic_capture = True
			world.img_render(world.display, count)
			world.depth_render(count)
			world.semantic_render(count)

			if(not count%4):
				world.save(count//4)
				world.depth_save(count//4)
				world.semantic_save(count//4)
		...
		if world is not None:
			...
			world.depth_camera.destroy()
			world.semantic_camera.destroy()
			world.destroy()
